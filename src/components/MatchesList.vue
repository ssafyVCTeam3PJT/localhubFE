...
<template>
  <div class="flex-1 bg-gray-50/50 h-full overflow-y-auto pb-10">
    ...
    <div class="max-w-[720px] mx-auto px-4 mt-6 flex flex-col gap-3">
      ...
      <div class="relative">
        <input
          ...
          class="w-full bg-white border border-gray-200 rounded-xl pl-10 pr-4 py-2.5 text-sm focus:outline-none focus:border-[#006c49] shadow-2xs text-gray-800"
          v-model="searchQuery"
        />
        <Search :size="16" class="text-gray-400 absolute left-3.5 top-3.5" />
      </div>

      <!-- Sports Category Filter Pill Scroller -->
      <div class="flex gap-2 overflow-x-auto no-scrollbar pb-1 scroll-smooth">
        <button
          v-for="sport in sportsFilters"
          :key="sport"
          @click="activeFilter = sport"
          :class="`px-4 py-1.5 rounded-full text-xs font-bold border transition-all whitespace-nowrap cursor-pointer ${
            activeFilter === sport
      ...
      <div class="space-y-4 mt-1">
        <div v-if="filteredPosts.length === 0" class="bg-white rounded-2xl border border-gray-100 p-12 text-center shadow-xs">
          ...
          <button
            ...
            class="mt-4 bg-[#10b981] hover:bg-[#006c49] text-white font-bold text-xs px-4 py-2 rounded-xl transition-all inline-flex items-center gap-1 cursor-pointer"
          >
        ...
        <template v-else>
          <div
            ...
            class="bg-white rounded-2xl border border-gray-100 p-4 shadow-2xs hover:shadow-xs transition-all duration-200 cursor-pointer flex gap-4 relative overflow-hidden group"
          >
            <!-- Thumbnail Image left -->
            <div class="w-20 h-20 md:w-24 md:h-24 bg-gray-100 rounded-xl overflow-hidden shrink-0">
              <img
            ...
            <div class="flex-1 min-w-0 flex flex-col justify-between">
              <div>
                ...
                <div class="flex justify-between items-center mb-1">
                  <span :class="`text-[9px] font-extrabold uppercase tracking-widest px-2 py-0.5 rounded-sm ${
                  ...
                  <!-- Joined badge marker -->
                  <span v-if="joinedPostIds.includes(post.id)" class="bg-[#006c49] text-white text-[9px] font-bold px-2 py-0.5 rounded-sm flex items-center gap-0.5 shadow-2xs">
                    <Check :size="8" /> 가입됨
                  </span>
                </div>

                <!-- Post Title -->
                <h3 class="font-headline-sm text-sm md:text-base font-bold text-gray-900 group-hover:text-[#006c49] transition-colors leading-snug line-clamp-1">
                  {{ post.title }}
                </h3>

                <!-- Map Location -->
                <p class="text-xs text-gray-500 mt-1 flex items-center gap-0.5 truncate">
              ...
              <div class="flex justify-between items-center border-t border-gray-50 pt-2 mt-2">
                <div class="flex items-center gap-2 text-[10px] text-gray-400 font-semibold">
                  <span class="flex items-center gap-0.5 bg-gray-100 text-gray-600 px-1.5 py-0.5 rounded-sm">
                    <Users :size="10" /> {{ post.joinedCount }} / {{ post.maxCount }}명
                  </span>
                  <span>조회 {{ post.views }}</span>
                </div>
                
                <span class="text-[10px] font-bold text-[#006c49] flex items-center gap-0.5">
                  자세히 보기
